        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-purple-950 p-4 md:p-8">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex justify-end gap-4 mb-8">
                    <LanguageToggle />
                    <Button variant="ghost" size="icon" onClick={toggleSound} className="text-slate-400 hover:text-white">
                        {soundEnabled ? <Volume2 className="w-5 h-5" /> : <VolumeX className="w-5 h-5" />}
                    </Button>
                    <Button variant="ghost" size="icon" onClick={toggleDarkMode} className="text-slate-400 hover:text-white">
                ...
                <motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
                    ...
                    <div className="flex justify-center gap-2 mt-4">
                        ...
                        <Badge variant="outline" className="text-green-400 border-green-500/50">GBIF</Badge>
                        <Badge variant="outline" className="text-purple-400 border-purple-500/50">DwC-DP</Badge>
                    </div>
                </motion.div>
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                        ...
                        <Card className="bg-slate-800/50 border-slate-700 backdrop-blur">
                            ...
                            <CardContent className="space-y-6">
                                <div className="space-y-2">
                                    <Label htmlFor="name" className="text-slate-300">{t('start.playerNameLabel')}</Label>
                                    <Input
                                        id="name"
                                ...
                                <Button onClick={handleStart} disabled={!playerName.trim()} size="lg" className="w-full py-6 text-lg bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700">
                                    ...
                                    {levels.map((level, idx) => {
                                        ...
                                        return (
                                            <motion.button
                                                ...
                                                className={`p-4 rounded-xl bg-gradient-to-br ${level.color} bg-opacity-10 border border-slate-600/50 hover:border-slate-500 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed text-left ${!unlocked ? 'opacity-30' : ''}`}
                                            >
                                                ...
                                                <p className="text-xs text-slate-300">{t(level.descKey)}</p>
                                            </motion.button>
                                        );
                                    })}
                                </div>
                                ...
                                >
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <span className="font-semibold text-white">{t('start.createDataPackage')}</span>
                                    </div>
                                    <p className="text-xs text-slate-300">{t('start.importOwnData')}</p>
                                </motion.button>
                            </CardContent>
                        </Card>
                    </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="space-y-6">
                                        ...
                                        {leaderboard.slice(0, 5).map((entry, idx) => (
                                            <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-slate-700/30">
                                                <div className="flex items-center gap-2">
                                                    <span className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold ${idx === 0 ? 'bg-yellow-500 text-black' : idx === 1 ? 'bg-slate-400 text-slate-900' : idx === 2 ? 'bg-orange-600 text-white' : 'bg-slate-600 text-white'}`}>
                                                        {idx + 1}
                                                    </span>
                                                    <span className="text-sm text-white truncate max-w-[100px]">{entry.name}</span>
                                                </div>
                                                <span className="text-sm text-yellow-400 font-mono">{entry.score}</span>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
                                    <p className="text-center text-slate-500 py-4">{t('start.beFirst')}</p>
                                )}
                            </CardContent>
                        </Card>
                        ...
                        <Card className="bg-slate-800/50 border-slate-700 backdrop-blur">
                            ...
                            <CardContent className="space-y-2">
                                {[
                                    ...
                                    <a key={link.href} href={link.href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-2 rounded-lg hover:bg-slate-700/50 text-slate-300 hover:text-white transition-colors">
                        ...
                        {/* How to Play */}
                        <Button variant="outline" className="w-full border-slate-600 text-slate-300 hover:bg-slate-700" onClick={() => setShowTutorial(!showTutorial)}>
                            <HelpCircle className="w-4 h-4 mr-2" />
                            {t('start.howToPlay')}
                        </Button>

                        {showTutorial && (
                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="p-4 rounded-xl bg-slate-800/80 border border-slate-700">
                                <h4 className="font-semibold text-white mb-2">{t('start.tutorial.title')}</h4>
                                ...
                                <p className="text-xs text-slate-500 mt-3">{t('start.tutorial.time')}</p>
                            </motion.div>
                        )}
                    </motion.div>
                </div>
                ...
                {/* Footer */}
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="text-center mt-12 text-slate-500 text-sm">
