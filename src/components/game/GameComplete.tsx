...
<body>
    <div class="certificate">
        ...
        <p style="font-size: 16px; margin-top: 30px;">Pokonałeś Chaos Validator i uratowałeś dane AMUNATCOLL!</p>
        <div class="score">
            <strong>Total Score:</strong> ${gameState?.totalScore || 0} points
        </div>
        <div style="margin-top: 40px; font-size: 14px; color: #666;">
            Completed: ${new Date().toLocaleDateString('pl-PL')}
        </div>
    </div>
</body>
</html>
            ...
            <div className="max-w-4xl w-full">
                        ...
                        >
                            {[...Array(50)].map((_, i) => (
                                <motion.div
                                    ...
                                    style={{ 
                                        backgroundColor: ['#fbbf24', '#ef4444', '#8b5cf6', '#06b6d4', '#10b981'][i % 5]
                                    }}
                                />
                            ))}
                        </motion.div>
                ...
                >
                    ...
                    <h1 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-2">
                        Gratulacje, {gameState?.playerName || 'Data Ranger'}!
                    </h1>
                    <p className="text-xl text-gray-600 dark:text-slate-400">
                        Ukończyłeś wszystkie misje DwC Data Quest!
                    </p>
                </motion.div>
                ...
                >
                    ...
                    <Card className="bg-white/90 border-gray-200 dark:bg-slate-800/80 dark:border-slate-700 backdrop-blur">
                        <CardContent className="pt-6 text-center">
                            ...
                            <div className="text-3xl font-bold text-gray-900 dark:text-white mb-1">
                                {gameState?.levelsCompleted?.length || 0}/4
                            </div>
                        ...
                        <CardContent>
                            <div className="space-y-3">
                                {[
                                    ...
                                    <div 
                                        key={level.id}
                                        className="flex items-center gap-4 p-3 rounded-lg bg-gray-100 border border-gray-200 dark:bg-slate-700/50 dark:border-slate-600"
                                    >
                                        <span className="text-2xl">{level.icon}</span>
                                        <div className="flex-1">
                                            <div className="font-semibold text-gray-900 dark:text-white">{level.name}</div>
                                            <div className="text-sm text-gray-600 dark:text-slate-400">
                                                {gameState?.levelScores?.[level.id] || 0} points
                                            </div>
                                        </div>
                ...
                >
                    ...
                    <Button
                        onClick={() => {
                            const text = `Ukończyłem DwC Data Quest z wynikiem ${gameState?.totalScore || 0} punktów! 🏆`;
                    ...
                    <Button
                        onClick={onRestart}
                        variant="outline"
                        className="w-full border-indigo-300 text-indigo-600 hover:bg-indigo-50 dark:border-indigo-600 dark:text-indigo-400 dark:hover:bg-indigo-500/10"
                        size="lg"
                    >
                ...
                >
                    <p className="text-sm text-gray-600 dark:text-slate-400">
                        Dziękujemy za grę! Teraz jesteś gotowy do publikacji danych w GBIF!
                    </p>
                </motion.div>
            </div>
        </div>
